...
for i in $(seq 1 10); do
    while true; do
        ...
        [[ -z "$ns" ]] && break 2
        valid_ip "$ns" && { NAMESERVERS+=("$ns"); break; } || echo_error "IP tidak valid. Masukkan IPv4 atau IPv6."
    done
done
...
optimasi_bind() {
    echo_info "Mengatur performa tinggi BIND..."
...
EOF
    echo_status "Optimasi BIND selesai"
}
